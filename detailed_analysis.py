...
def analyze_demos(dataset_path):
    ...
    for i, parquet_file in enumerate(parquet_files):
        try:
            ...
            for ep in episodes:
                ...
                timestamps = [
                    ts[0] if isinstance(ts, (list, np.ndarray)) else ts
    ...
    # 打印表格
    print(
        f"{'Demo ID':<8} {'帧数':<8} {'时长(s)':<10} {'FPS':<8} {'平均dt(s)':<12} {'dt标准差':<10} {'文件'}"
    )
    print("-" * 80)

    for ep_id, info in sorted_episodes[:30]:  # 显示前30个
        print(
            f"{ep_id:<8} {info['frames']:<8} {info['duration']:<10.2f} {info['estimated_fps']:<8.1f} "
    ...
    if all_episodes_info:
        ...
        dts_list = [
            info["avg_dt"] for info in all_episodes_info.values() if info["avg_dt"] > 0
        ]
        ...
        print(
            f"  总时长: {sum(durations_list):.2f} 秒 ({sum(durations_list) / 60:.1f} 分钟)"
        )
